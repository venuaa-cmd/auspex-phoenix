 ...
 * Integrated Ledger Math + Governor Audits + Personnel Roster
 */
const DomainBudgetManagement = ({ domains = [], toWords, formatCurrency, isSuperAdmin }) => {
    ...
    // --- 4.5 SEARCH & SORT STATES ---
    // Personnel Controls
    const [sortType, setSortType] = useState('budget'); 
    const [searchTerm, setSearchTerm] = useState('');
    
    // Domain Controls (Restored & Integrated)
    const [domainSearch, setDomainSearch] = useState('');
    const [domainSort, setDomainSort] = useState('name'); // 'name', 'allocated', 'spent'

    // --- DEFENSIVE PARSING PROTOCOL ---
    ...
    // --- NEW: DOMAIN SEARCH & SORT RESOLVER ---
    ...
    // --- NEW: FILTERED & SORTED PERSONNEL RESOLVER ---
    ...
    // --- 9. STAGE 1: INITIATE AI ANALYSIS ---
    const handleInitiateAnalysis = async () => {
        ...
        try {
            ...
            setPreviewData(strategy); setMarketAnalysis(strategy.market_intel); setDeploymentAnalysis(strategy.deployment_rationale);
        } catch (err) { alert("Analysis Failed: " + err.message); }
        finally { setIsUpdating(false); }
    };

    // --- 10. STAGE 2: FINAL AUTHORIZATION ---
    const handleFinalCommit = async () => {
        if (!previewData) return;
        setIsUpdating(true);
        try {
            const totalDeployment = previewData.allocations.reduce((sum, a) => sum + a.budget, 0);
            await supabase.from('fund_ledger').insert([{ amount: totalDeployment, type: 'DEBIT', description: `Tactical Deployment: ${new Date().toLocaleDateString()}`, reference: 'AI_REBALANCE_PROTOCOL' }]);
            ...
            for (const alloc of previewData.allocations) {
                ...
                for (const mAlloc of (alloc.manager_assignments || [])) {
                    ...
                    if (manager) {
                        ...
                        await supabase.from('fund_managers').update({ budget: { ...existingBudget, annual: (existingBudget.annual || 0) + mAlloc.amount } }).eq('id', manager.id);
                        const curW = managerWallets.find(w => w.manager_id === manager.id && w.domain_name === alloc.domain);
                        await supabase.from('manager_domain_allocations').upsert({ manager_id: manager.id, domain_name: alloc.domain, allocated_amount: (curW?.allocated_amount || 0) + mAlloc.amount, last_updated: new Date() });
                    }
                }
            }

            await supabase.from('erp_strategic_audits').insert([{ market_intelligence: previewData.market_intel, deployment_rationale: previewData.deployment_rationale }]);
            alert("TREASURY_DEPLOYED: Ledger updated & Audits Archived.");
            setPreviewData(null); await fetchSystemData(); 
    ...
    return (
        <div className="space-y-10 font-manrope text-white pb-40 relative">
            ...
            {/* 3. NEURAL_DOMAIN_UTILIZATION_AUDIT */}
                ...
                <div className="mb-8 flex flex-col md:flex-row justify-between items-center gap-6 bg-white/[0.03] p-6 border border-cyan-500/10 rounded-xl shadow-xl">
                    <div className="flex gap-3">
                        {['name', 'allocated', 'spent'].map(type => (
                            <button 
                                key={type} 
                                onClick={() => setDomainSort(type)} 
                                className={`px-6 py-2 text-[10px] font-black uppercase border transition-all ${domainSort === type ? 'bg-cyan-600 text-white border-cyan-500' : 'text-slate-500 border-white/10 hover:border-cyan-500/30'}`}
                            >
                    ...
                    <div className="relative w-full md:w-80">
                        ...
                        <input 
                            type="text" 
                            placeholder="SEARCH_DOMAIN_NODE..." 
                            value={domainSearch} 
                            onChange={e => setDomainSearch(e.target.value)} 
                            className="w-full pl-10 pr-4 py-3 bg-black border border-cyan-500/30 text-[10px] font-black text-white outline-none focus:border-cyan-400 transition-all" 
                        />
                    </div>
                </div>
                    ...
                    {filteredAndSortedDomains.map(d => (
                        <div key={d.id} className="space-y-4 group">
                            <div className="flex justify-between items-end">
                                ...
                                <div className="text-right"><span className={`text-[12px] font-black font-mono ${d.rawUtil > 90 ? 'text-rose-400' : 'text-cyan-400'}`}>{d.rawUtil.toFixed(1)}%</span></div>
                            </div>
            ...
            <div className="mt-20 border-t border-white/5 pt-10">
                ...
                <div className="mb-8 flex flex-col md:flex-row justify-between items-center gap-6 bg-white/[0.03] p-6 border border-white/5 rounded-xl shadow-xl">
                    <div className="flex gap-3">
                        {['name', 'budget', 'utilization'].map(type => (
                            <button 
                                ...
                                className={`px-6 py-2 text-[10px] font-black uppercase border transition-all ${sortType === type ? 'bg-[#FFD700] text-black border-[#FFD700]' : 'text-slate-500 border-white/10 hover:border-white/30'}`}
                            >
                                {type.replace('_', ' ')}
                            </button>
                        ))}
                    ...
                    <div className="relative w-full md:w-80">
                        ...
                        <input 
                            type="text" 
                            placeholder="SEARCH_BY_MANAGER_NAME..." 
                            value={searchTerm} 
                            onChange={e => setSearchTerm(e.target.value)} 
                            className="w-full pl-10 pr-4 py-3 bg-black border border-white/10 text-[10px] font-black text-white outline-none focus:border-[#FFD700] transition-all" 
                        />
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {filteredManagers.map(manager => (
                        <div key={manager.id} className="bg-black/40 border border-white/5 p-6 rounded-xl hover:border-cyan-500/50 transition-all text-left group">
                            ...
                            <button onClick={() => setSelectedManagerId(manager.id)} className="w-full py-2 bg-white/5 border border-white/10 text-[9px] font-black uppercase hover:bg-cyan-600 hover:text-white transition-all">OPEN_AUDIT →</button>
                        </div>
                    ))}
                </div>
            </div>
